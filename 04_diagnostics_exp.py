...
# samples are stored as (alpha, phi, psi) in log-space — phi=log(beta), psi=log(gamma)
...
# the sampler worked in log-space; we plot the actual physical quantities
beta_s  = np.exp(phi_s)
gamma_s = np.exp(psi_s)

# trace plot
# 3 params now — same thing as linear: want fuzzy caterpillar, no drift, no flat stretches
# drift = not converged. flat = stuck. a good trace looks like white noise around a mean
...
axes[1].set_ylabel("beta (recovered)")
...
axes[2].set_ylabel("gamma (recovered)")
...
# delta H
# H = -log P(theta|D) + p^2/2 (hamiltonian)
# leapfrog conserves H so delta_H = H_old - H_new should be near 0
# systematically negative mean = epsilon too big, integration diverging
...
# MCMC samples are correlated — each sample depends on the previous one
# ESS = N / (1 + 2 * sum_k rho_k) counts how many truly independent draws we have
# we want ESS > 400 per param. hmc tends to give much higher ESS than random-walk samplers
# because gradient-guided proposals jump further and decorrelate faster
...
# autocorrelation
# rho_k = correlation(sample_i, sample_{i+k}) — how fast does the chain forget where it was?
# drops to 0 quickly = good mixing = high ESS
# stays elevated for many lags = chain is crawling, not exploring
max_lag = 100
...
# the shape of what we sampled — plotted in original (non-log) space so values are interpretable
# this is the distribution over (alpha, beta, gamma) that the data supports given our priors
# spread = uncertainty in each param, which propagates into the RUL distribution in 05
fig, axes = plt.subplots(1, 3, figsize=(15, 4))

for ax, vals, name, color in zip(
    axes,
    [alpha_s, beta_s, gamma_s],
    ["alpha", "beta (recovered)", "gamma (recovered)"],
    ["steelblue", "tomato", "seagreen"]
):
    ax.hist(vals, bins=60, color=color, edgecolor="white", linewidth=0.3, density=True)
    ax.axvline(vals.mean(), color="red",    linewidth=1.2, linestyle="--", label=f"mean={vals.mean():.4f}")
    ax.axvline(np.percentile(vals,  5), color="orange", linewidth=1, linestyle=":", label="5/95%")
...
print(f"gamma: mean={gamma_s.mean():.4f}  std={gamma_s.std():.4f}  90% CI=[{np.percentile(gamma_s,5):.4f}, {np.percentile(gamma_s,95):.4f}]")
