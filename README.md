...
where `α` is starting health and `β` is the degradation rate (assuming linear (which for more complex/realsitic models you shouldn't)). Sensor readings are noisy observations of true health:
...
Computing this exactly requires integrating over all possible parameter values which is analytically intractable. So instead of computing it, we **sample** from it using **Hamiltonian Monte Carlo (HMC)**.
...
The dataset is not included in this repo. It can be downloaded from the [NASA website](https://data.nasa.gov/dataset/cmapss-jet-engine-simulated-data).
